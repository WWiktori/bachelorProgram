...
from tabulate import tabulate  # Для красивих таблиць

# --- Завантаження даних ---
full_df = pd.read_excel('Worldwide.xlsx')
full_df["date"] = pd.to_datetime(full_df[["year", "month", "day"]])

# Фільтрація по Україні (регістр може бути різний)
ukraine_df = full_df[full_df["countriesAndTerritories"].str.upper() == "UKRAINE"]

# Сортування за датою
ukraine_df = ukraine_df.sort_values("date")

# Вивід перших 5 записів
print(ukraine_df.head())

print(f"\nДані з {ukraine_df['date'].min().date()} по {ukraine_df['date'].max().date()}\n")
...
for idx, (start_date, end_date, label) in enumerate(periods):
    ...
    # Підготовка даних
    period_data = ukraine_df[(ukraine_df['date'] >= start_date) & (ukraine_df['date'] <= end_date)]
